# pocket-ui/src/main/res/drawable/ic_settings_checkable.xml
## What this is
This file is the `ic_settings_checkable` vector icon (a `vector` XML: resolution-independent line art Android rasterizes at any density, so one file serves all screens). It draws the gear glyph used on buttons, rows, and toolbars across the app. 
## How it fits
