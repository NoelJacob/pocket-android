# pocket-ui/src/main/res/drawable/ic_pkt_archive_checked.xml
## What this is
This file is the `ic_pkt_archive_checked` vector icon (a `vector` XML: resolution-independent line art Android rasterizes at any density, so one file serves all screens). It draws the archive box glyph used on buttons, rows, and toolbars across the app. 
## How it fits
