# pocket-ui/src/main/res/drawable/ic_pkt_sort_line.xml
## What this is
This file is the `ic_pkt_sort_line` vector icon (a `vector` XML: resolution-independent line art Android rasterizes at any density, so one file serves all screens). It draws the sort lines glyph used on buttons, rows, and toolbars across the app. 
## How it fits
