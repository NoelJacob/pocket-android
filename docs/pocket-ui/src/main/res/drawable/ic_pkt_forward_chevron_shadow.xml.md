# pocket-ui/src/main/res/drawable/ic_pkt_forward_chevron_shadow.xml
## What this is
This file is the `ic_pkt_forward_chevron_shadow` vector icon (a `vector` XML: resolution-independent line art Android rasterizes at any density, so one file serves all screens). It draws the forward chevron, chevron, chevron with shadow glyph used on buttons, rows, and toolbars across the app. 
## How it fits
