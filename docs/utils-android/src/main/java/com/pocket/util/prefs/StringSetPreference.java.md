# utils-android/src/main/java/com/pocket/util/prefs/StringSetPreference.java
...
- `StringSetPreference` (interface, line 5) — core type of this file; callers reference it by name.
- `get` (fun, line 10) — The current value. If {@link #set(Set)} has never been called, it returns the default value. Use {@link #isSet()} if you need to known.
- `set` (fun, line 11) — entry point other code calls; see callers for context.
