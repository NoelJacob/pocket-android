...
This is the Spanish (generic) translation of the app's string table: the same keys as `values/strings.xml`, with translated values (464 entries). Translators work through Smartling (see the `smartling.*` comments in the base file); translators never add or remove keys here.
