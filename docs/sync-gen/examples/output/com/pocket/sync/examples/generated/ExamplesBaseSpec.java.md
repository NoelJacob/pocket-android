# sync-gen/examples/output/com/pocket/sync/examples/generated/ExamplesBaseSpec.java

## What this is

Checked-in codegen output (never edit by hand): this file shows the near-ready BaseSpec subclassed by hand-written example specs. Like all files under examples/output, it is produced from the example schema (examples.graphqls plus queries.graphql plus examples-usage.txt) by running ExamplesGenerator, and it exists so developers can read real emitted code without running codegen themselves.
...
- `things()` — one emitted accessor/field of this generated type, mirroring its schema declaration
- `actions()` — one emitted accessor/field of this generated type, mirroring its schema declaration
- `derive()` — one emitted accessor/field of this generated type, mirroring its schema declaration
