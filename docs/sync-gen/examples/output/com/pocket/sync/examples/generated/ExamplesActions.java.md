# sync-gen/examples/output/com/pocket/sync/examples/generated/ExamplesActions.java

## What this is

Checked-in codegen output (never edit by hand): this file shows the actions registry (Spec.actions() implementation). Like all files under examples/output, it is produced from the example schema (examples.graphqls plus queries.graphql plus examples-usage.txt) by running ExamplesGenerator, and it exists so developers can read real emitted code without running codegen themselves.
...
- `action_example()` — one emitted accessor/field of this generated type, mirroring its schema declaration
- `exampleMutation()` — one emitted accessor/field of this generated type, mirroring its schema declaration
- `no_effect_action()` — one emitted accessor/field of this generated type, mirroring its schema declaration
- `resolved_action()` — one emitted accessor/field of this generated type, mirroring its schema declaration
