# sync-pocket/src/main/java/com/pocket/sdk/api/value/Password.java
...
- `Password` (class, line 18) — Passwords should never ever be stored client side, even encrypted. Just don't do it.
- `equals` (fun, line 29) — entry point other code calls; see callers for context.
- `hashCode` (fun, line 36) — entry point other code calls; see callers for context.
- `toString` (fun, line 41) — entry point other code calls; see callers for context.
- `toJsonValue` (fun, line 45) — entry point other code calls; see callers for context.
...
Names you will also see here: `Source`, `Space`.
